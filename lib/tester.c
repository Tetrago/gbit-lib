...
static void dump_state(struct state* state)
{
	printf(
	    ...
	    state->reg16.DE,
	    state->reg16.HL,
	    BIT(state->reg16.AF, 7),
	    BIT(state->reg16.AF, 6),
	    BIT(state->reg16.AF, 5),
	    BIT(state->reg16.AF, 4),
	    state->halted,
	...
	for (int i = 0; i < state->num_mem_accesses; i++)
		printf("  Mem %s: addr=%04x val=%02x\n",
		       ...
		       state->mem_accesses[i].addr,
		       state->mem_accesses[i].val);
	printf("\n");
}
...
static void dump_op_state(struct test_inst* inst, struct op_state* op_state)
{
	...
	if (inst->bit_bitpos != -1) printf("bit: %d\n", op_state->bit);
}
...
static int states_mem_accesses_eq(struct state* s1, struct state* s2)
{
	...
	if (s1->num_mem_accesses != s2->num_mem_accesses) return 0;
	...
	qsort(s1->mem_accesses,
	      s1->num_mem_accesses,
	      sizeof(struct mem_access),
	      mem_access_cmp);
	qsort(s2->mem_accesses,
	      s2->num_mem_accesses,
	      sizeof(struct mem_access),
	      mem_access_cmp);
...
static void assemble(u8* out, struct test_inst* inst, struct op_state* op_state)
{
	...
	if (inst->reg8_bitpos >= 0) opcode |= op_state->reg8 << inst->reg8_bitpos;
	...
	if (inst->cond_bitpos >= 0) opcode |= op_state->cond << inst->cond_bitpos;
	if (inst->bit_bitpos >= 0) opcode |= op_state->bit << inst->bit_bitpos;

	if (inst->is_cb_prefix) out[idx++] = 0xcb;

	out[idx++] = opcode;

	if (inst->imm_size >= 1) out[idx++] = op_state->imm & 0xff;
	if (inst->imm_size >= 2) out[idx++] = (op_state->imm >> 8) & 0xff;
}
...
static int test_instruction(struct test_inst* inst)
{
	...
	do
	{
		...
		if (opcode != last_op) last_op_had_failure = false;

		if (last_op_had_failure) continue;
		...
		{
			had_failure = true;
			if (!flags->keep_going_on_mismatch) return 1;
		}
	}
                             ...
                             const char* prefix)
{
	...
	for (size_t i = 0; i < num_instructions; i++)
	{
		...
		if (flags->print_tested_instruction)
		{
			...
			if (insts[i].is_cb_prefix) printf("(CB prefix)");
			printf("\n");
		}

		if (!insts[i].enabled)
		{
			if (flags->print_tested_instruction) printf(" Skipping\n");
			continue;
		}

		failure = test_instruction(&insts[i]);
		if (failure && !flags->keep_going_on_mismatch) return 1;

		if (flags->print_tested_instruction)
			printf(" Ran %lu permutations\n", num_tests);

		if (!failure) num_instructions_passed++;
		num_instructions_tested++;
		num_tests = 0;
	}

	if (flags->print_tested_instruction) printf("\n");
	...
	if (num_instructions_passed != num_instructions_tested)
		printf(", %zu passed and %zu failed",
...
static bool is_valid_op(u8 op)
{
	...
	for (size_t i = 0; i < sizeof(invalid_ops); i++)
		if (invalid_ops[i] == op) return false;
                           ...
                           const char* prefix)
{
	...
	{
		if (op_table[op]) num_tested_ops++;
		if (is_valid(op)) total_valid_ops++;
	}
...
static int test_all_instructions(void)
{
	...
	if (test_instructions(num_instructions, instructions, "")) return 1;
...
extern void init(size_t, uint8_t*);
extern void set_state(struct state*);
extern void get_state(struct state*);
extern int step(void);

struct tester_operations get_operations()
{
	struct tester_operations ops;
	ops.init      = init;
	ops.set_state = set_state;
	ops.get_state = get_state;
	ops.step      = step;
	return ops;
}
